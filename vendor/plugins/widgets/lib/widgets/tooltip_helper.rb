module Widgets
  module TooltipHelper
    ...
    def tooltip(name=nil, opts={}, &proc)
      ...
      # e.g. <%= tooltip :name => '...', :tooltip_class => 'mytooltip_class' %>
      opts[:tooltip_class] ||= 'tooltip'
      opts[:tooltip_content_class] ||= 'tooltip_content'
 
      result = ''
      # Tee: moved inline CSS to main.css  # result << tooltip_css
    ...
    def render_tooltip(name, content, opts)
      html = tag('div', {:id => "tooltip_#{opts[:id]}", :class=>opts[:tooltip_class], :style => 'display:none'}, true)
      html << tag('div', {:id => "tooltip_content_#{opts[:id]}", :class=>opts[:tooltip_content_class]},true)
      html << content
