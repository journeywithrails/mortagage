  ...
  module Finder
    ...
    def find_page(options = {})
      ...
      finder_options = { :order => build_order_clause(options[:sorting]),
                         ...
                         :group => group_for_finder,
                         :joins => joins_for_finder,
                         :include => options[:count_includes]}

      # NOTE: we must use :include in the count query, because some conditions may reference other tables
      #~ count = klass.count(finder_options.reject{|k,v| [:order].include? k}) ===== before change
      
        count = klass.count(finder_options.reject{|k,v| [:order].include? k}) #after change
    ...
    def group_for_finder
      case group_for_collection
      when String
        group_for_collection 
      when Array
        group_for_collection
      else
        group_for_collection
