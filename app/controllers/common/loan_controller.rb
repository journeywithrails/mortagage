class Common::LoanController < ApplicationController
  ...
  active_scaffold :loan do |config|
    config.columns = [:property, :borrower_person, :co_borrower_person, :loan_type, :loan_rank, :is_active, :loan_status,
                      ...
                      :adjustment_cap, :payment_adjustment_period, :biweekly_payments]
    ...
    config.columns[:loan_status].clear_link
    config.columns[:loan_type].clear_link
